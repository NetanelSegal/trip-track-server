...
interface ServerToClientEvents {
	...
	locationUpdated: (userId: string, location: { lat: number; lon: number }) => void;
	tripStatusChanged: (tripId: string, status: string) => void;
	error: (data: { errorDetails: Record<string, any>; message: string } | string) => void;
}

interface ClientToServerEvents {
	joinTrip: (tripId: string) => void;
	updateLocation: (tripId: string, { lon, lat }: { lon: number; lat: number }) => void;
...
export { SocketServer, SocketType, ClientToServerEvents, InterServerEvents, ServerToClientEvents, SocketData };
