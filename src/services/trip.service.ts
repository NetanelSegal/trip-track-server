...
interface TripService {
	...
	updateTrip: (userId: string, tripId: string, data: Types['Trip']['Model']) => Promise<Types['Trip']['Model']>;
	getTripById: (id: string) => Promise<Types['Trip']['Model']>;
	getTrips: (userId: string, page?: number, limit?: number) => Promise<Types['Trip']['Model'][]>;
...
export const mongoUpdateTrip: TripService['updateTrip'] = async (userId, tripId, data) => {
	try {
...
export const mongoGetTrips: TripService['getTrips'] = async (userId, page = 1, limit = 10) => {
	try {
...
export const mongoDeleteTrip: TripService['deleteTrip'] = async (userId, tripId) => {
	try {
		const trip = await Trip.findById(tripId);
