...
export const getAddressSuggestions = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		if (googleRespone.data.status === 'REQUEST_DENIED') {
			throw new AppError('AppError', googleRespone.data.error_message, 500, 'Google');
		}
...
export const getGeocodeForAddress = async (req: Request, res: Response, next: NextFunction) => {
	try {
